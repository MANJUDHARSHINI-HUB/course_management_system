...
function buildStudentRow(student) {
  ...
  var action = active ?
    '<button type="button" class="btn btn-ghost btn-sm force-logout-student" data-username="' + student.username + '"><i class="fa-solid fa-right-from-bracket"></i> Logout</button>' :
    '<span class="muted-action">Not logged in</span>';
  var loginDetails = student.activeRecord ?
    '<small style="color:var(--muted-dim);display:block;margin-top:4px;">Login: ' + new Date(student.activeRecord.loggedInAt).toLocaleString() + '<br>Last active: ' + new Date(student.activeRecord.lastSeen).toLocaleTimeString() + '</small>' :
    '<small style="color:var(--muted-dim);display:block;margin-top:4px;">Not currently logged in</small>';
  var row = "<tr>";
  row += "<td>" + student.name + " <br><small style=\"color:var(--muted-dim);\">" + student.id + "</small><br><small style=\"color:var(--muted-dim);\">" + student.email + "</small>" + loginDetails + "</td>";
  row += "<td>" + student.dept + "</td>";
  row += '<td><span class="pill ' + statusClass + '">' + statusText + '</span></td>';
  row += '<td class="table-actions">' + action + '</td>';
  row += "</tr>";
  return row;
}
    ...
    if (studentTableBody !== null) {
      ...
      if (realStudents.length > 0) {
        ...
        for (var s = 0; s < studentsData.length; s++) {
          studentRows += buildStudentRow({name: studentsData[s].name, id: studentsData[s].id, email: "", dept: studentsData[s].dept, username: "", active: false, activeRecord: null});
        }
      }
